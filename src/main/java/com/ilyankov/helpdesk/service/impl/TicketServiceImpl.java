...
import com.ilyankov.helpdesk.converter.impl.TicketConverter;
...
public class TicketServiceImpl implements TicketService {
	...
	private final EditConverter editConverter;
	...
	public List<TicketDto> getAllByRole(User user, Integer page) {
		Integer firstResult = Сomputation.getFirstResult(page, Quantity.TEN.label);
	...
	public List<TicketDto> getAllByRole(User user) {
		Map<Role, Supplier<List<TicketDto>>> strategyMap = Map.of(Role.Employee,
				() -> converter.toDto(ticketRepository.getAllByEmployee(user)), Role.Manager,
				() -> converter.toDto(ticketRepository.getAllByManager(user)), Role.Engineer,
				() -> converter.toDto(ticketRepository.getAllByEngineer(user)));
	...
	public Ticket edit(User user, TicketDto ticketDto) {
		...
		editConverter.edit(ticket, ticketDto);
	...
	public List<TicketDto> getByRoleMyTicket(User user, Integer page) {
		...
		Map<Role, Supplier<List<TicketDto>>> strategyMap = Map.of(Role.Employee,
				() -> converter.toDto(ticketRepository.getAllByEmployee(user, firstResult, Quantity.TEN.label)),
				Role.Manager,
				() -> converter.toDto(ticketRepository.getMyTicketByManager(user, firstResult, Quantity.TEN.label)),
				Role.Engineer,
				() -> converter.toDto(ticketRepository.getMyTicketByEngineer(user, firstResult, Quantity.TEN.label)));
	...
	public List<TicketDto> getByRoleMyTicket(User user) {
		Map<Role, Supplier<List<TicketDto>>> strategyMap = Map.of(Role.Employee,
				() -> converter.toDto(ticketRepository.getAllByEmployee(user)), Role.Manager,
				() -> converter.toDto(ticketRepository.getMyTicketByManager(user)), Role.Engineer,
				() -> converter.toDto(ticketRepository.getMyTicketByEngineer(user)));
	...
	public Long getCountMyByRole(User user) {
		Map<Role, Supplier<Long>> strategyMap = Map.of(Role.Employee,
				() -> ticketRepository.getCountAllByEmployee(user), Role.Manager,
				() -> ticketRepository.getCountMyTicketByManager(user), Role.Engineer,
				() -> ticketRepository.getCountMyTicketByEngineer(user));
	...
	List<TicketDto> getLimitTicket(Integer page, List<TicketDto> sortedList) {
		int firstResult = Сomputation.getFirstResult(page, Quantity.TEN.label);
		int limit = 0;
